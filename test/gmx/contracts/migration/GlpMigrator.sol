...
contract GlpMigrator is ReentrancyGuard, RoleModule {
    ...
    function setReducedMintBurnFeeBasisPoints(uint256 _reducedMintBurnFeeBasisPoints) external onlyConfigKeeper {
        ...
        eventEmitter.emitEventLog("SetReducedMintBurnFeeBasisPoints", eventData);
    }

    function migrate(uint256 totalGlpAmount, MigrationItem[] memory migrationItems)
        external
        payable
        nonReentrant
        withReducedRedemptionFees
    {
        ...
        for (uint256 i = 0; i < migrationItems.length; i++) {
            ...
            TokenUtils.depositAndSendWrappedNativeToken(dataStore, address(depositVault), migrationItem.executionFee);
            ...
            cache.depositKey = depositHandler.createDeposit(account, depositParams);
            ...
            eventEmitter.emitEventLog1("GlpMigration", Cast.toBytes32(cache.market.marketToken), eventData);
        }
    }

    function _redeemGlp(GlpRedemption memory redemptionInfo) internal returns (uint256) {
