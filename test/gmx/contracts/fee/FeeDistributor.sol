...
contract FeeDistributor is ReentrancyGuard, RoleModule {
    ...
    function claimFees(uint256 startIndexV2, uint256 endIndexV2) external nonReentrant onlyFeeDistributionKeeper {
    ...
    function swapFeesUsingV1(
        ...
        FeeSwapUtils.swapFeesUsingV1(
            dataStore, routerV1, bridgingToken, feeBatchKey, tokenIndex, path, swapAmount, minOut
        );
    }
    ...
    function _claimFeesV2(FeeBatch.Props memory feeBatch, address[] memory marketKeys, uint256 countV1, uint256 countV2)
        internal
        returns (FeeBatch.Props memory)
    {
        for (uint256 i; i < countV2; i++) {
            ...
            uint256 longTokenFeeAmount =
                FeeUtils.claimFees(dataStore, eventEmitter, market.marketToken, market.longToken, address(this));

            uint256 shortTokenFeeAmount =
                FeeUtils.claimFees(dataStore, eventEmitter, market.marketToken, market.shortToken, address(this));
