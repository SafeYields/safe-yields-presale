...
contract ExchangeRouter is IExchangeRouter, BaseRouter {
    ...
    function makeExternalCalls(
        ...
        externalHandler.makeExternalCalls(externalCallTargets, externalCallDataList, refundTokens, refundReceivers);
    }
    ...
    function simulateExecuteLatestDeposit(OracleUtils.SimulatePricesParams memory simulatedOracleParams)
        external
        payable
        nonReentrant
    {
    ...
    function executeAtomicWithdrawal(
        ...
        return withdrawalHandler.executeAtomicWithdrawal(account, params, oracleParams);
    }
        ...
        returns (bytes32)
    {
        ...
        return shiftHandler.createShift(account, params);
    }

    function cancelShift(bytes32 key) external payable override nonReentrant {
    ...
    function simulateExecuteShift(bytes32 key, OracleUtils.SimulatePricesParams memory simulatedOracleParams)
        external
        payable
        nonReentrant
    {
    ...
    function createOrder(IBaseOrderUtils.CreateOrderParams calldata params)
        ...
        returns (bytes32)
    {
        //console.log("ExchangeRouter - create Order");
        address account = msg.sender;
    ...
    function updateOrder(
        ...
        orderHandler.updateOrder(
            key, sizeDeltaUsd, acceptablePrice, triggerPrice, minOutputAmount, validFromTime, autoCancel, order
        );
    }
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i; i < markets.length; i++) {
            claimedAmounts[i] =
                MarketUtils.claimFundingFees(dataStore, eventEmitter, markets[i], tokens[i], account, receiver);
        }
    ...
    function claimCollateral(
        ...
        for (uint256 i; i < markets.length; i++) {
            claimedAmounts[i] = MarketUtils.claimCollateral(
                dataStore, eventEmitter, markets[i], tokens[i], timeKeys[i], account, receiver
            );
        }
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i; i < markets.length; i++) {
            claimedAmounts[i] =
                ReferralUtils.claimAffiliateReward(dataStore, eventEmitter, markets[i], tokens[i], account, receiver);
        }
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i; i < markets.length; i++) {
            claimedAmounts[i] =
                FeeUtils.claimUiFees(dataStore, eventEmitter, uiFeeReceiver, markets[i], tokens[i], receiver);
        }
