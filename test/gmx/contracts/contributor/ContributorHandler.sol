...
contract ContributorHandler is ReentrancyGuard, RoleModule, BasicMulticall {
    ...
    function setMaxTotalContributorTokenAmount(address[] memory tokens, uint256[] memory amounts)
        external
        nonReentrant
    ...
    function sendPayments() external nonReentrant onlyContributorDistributor {
        ...
        for (uint256 i; i < tokenCount; i++) {
            ...
            for (uint256 j; j < accountCount; j++) {
                ...
                eventEmitter.emitEventLog1("SendContributorPayment", Cast.toBytes32(account), eventData);
            }
        }
    ...
    function setContributorAmount(address account, address[] memory tokens, uint256[] memory amounts)
        external
        nonReentrant
        onlyContributorKeeper
    {
        ...
        for (uint256 i; i < tokens.length; i++) {
            ...
            eventData.uintItems.initItems(1);
            eventData.uintItems.setItem(0, "amount", amount);
            eventEmitter.emitEventLog1("SetContributorAmount", Cast.toBytes32(account), eventData);
        }
