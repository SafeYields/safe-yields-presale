...
contract GlvHandler is BaseHandler, ReentrancyGuard {
    ...
    function createGlvDeposit(address account, GlvDepositUtils.CreateGlvDepositParams calldata params)
        external
        globalNonReentrant
        onlyController
        returns (bytes32)
    ...
    function executeGlvDeposit(bytes32 key, OracleUtils.SetPricesParams calldata oracleParams)
        external
        globalNonReentrant
        onlyOrderKeeper
        withOraclePrices(oracleParams)
    {
        ...
        try this._executeGlvDeposit{ gas: executionGas }(key, glvDeposit, msg.sender) { }
    ...
    function createGlvWithdrawal(address account, GlvWithdrawalUtils.CreateGlvWithdrawalParams calldata params)
        external
        globalNonReentrant
        onlyController
        returns (bytes32)
    ...
    // @key glvDeposit key
    // @oracleParams prices for all markets in GLV are required
    function executeGlvWithdrawal(bytes32 key, OracleUtils.SetPricesParams calldata oracleParams)
        external
        globalNonReentrant
        onlyOrderKeeper
        withOraclePrices(oracleParams)
    {
        ...
        uint256 estimatedGasLimit =
            GasUtils.estimateExecuteGlvWithdrawalGasLimit(_dataStore, glvWithdrawal, marketCount);
        GasUtils.validateExecutionGas(_dataStore, startingGas, estimatedGasLimit);

        uint256 executionGas = GasUtils.getExecutionGas(_dataStore, startingGas);

        try this._executeGlvWithdrawal{ gas: executionGas }(key, glvWithdrawal, msg.sender) { }
        catch (bytes memory reasonBytes) {
            _handleGlvWithdrawalError(key, startingGas, reasonBytes);
        }
    }

    function _executeGlvWithdrawal(bytes32 key, GlvWithdrawal.Props memory glvWithdrawal, address keeper)
        external
        onlySelf
    {
        uint256 startingGas = gasleft();
    ...
    function simulateExecuteGlvDeposit(bytes32 key, OracleUtils.SimulatePricesParams memory params)
        external
        onlyController
    ...
    function simulateExecuteGlvWithdrawal(bytes32 key, OracleUtils.SimulatePricesParams memory params)
        external
        onlyController
        withSimulatedOraclePrices(params)
        globalNonReentrant
    ...
    function createGlvShift(GlvShiftUtils.CreateGlvShiftParams memory params)
        external
        globalNonReentrant
        onlyOrderKeeper
        returns (bytes32)
    ...
    function executeGlvShift(bytes32 key, OracleUtils.SetPricesParams calldata oracleParams)
        external
        globalNonReentrant
        onlyOrderKeeper
        withOraclePrices(oracleParams)
    {
        ...
        try this._executeGlvShift{ gas: executionGas }(key, glvShift, msg.sender) { }
        catch (bytes memory reasonBytes) {
    ...
    function _handleGlvShiftError(bytes32 key, bytes memory reasonBytes) internal {
        ...
        GlvShiftUtils.cancelGlvShift(dataStore, eventEmitter, key, reason, reasonBytes);
    }
