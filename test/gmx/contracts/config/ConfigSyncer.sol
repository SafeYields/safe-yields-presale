...
contract ConfigSyncer is ReentrancyGuard, RoleModule {
    ...
    function sync(address[] calldata markets, string[] calldata parameters)
        external
        ...
        nonReentrant
    {
        ...
        for (uint256 i; i < markets.length; i++) {
            ...
            bool syncConfigMarketParameterDisabled =
                dataStore.getBool(Keys.syncConfigMarketParameterDisabledKey(market, parameter));
            ...
            IRiskOracle.RiskParameterUpdate memory riskParameterUpdate =
                IRiskOracle(riskOracle).getLatestUpdateByParameterAndMarket(parameter, market);
