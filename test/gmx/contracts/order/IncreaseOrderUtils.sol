...
library IncreaseOrderUtils {
        ...
        returns (EventUtils.EventLogData memory)
    {
        ...
        console.log("validatePositionMarket");

        (address collateralToken, uint256 collateralIncrementAmount) = SwapUtils.swap(
        ...
        bytes32 positionKey = Position.getPositionKey(
            params.order.account(), params.order.market(), collateralToken, params.order.isLong()
        );
        ...
        console.log("params.minOracleTimestamp", params.minOracleTimestamp);

        console.log("params.order.updatedAtTime()", params.order.updatedAtTime());

        params.minOracleTimestamp = 1732933351;
