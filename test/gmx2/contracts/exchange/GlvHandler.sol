...
contract GLVHandler is BaseHandler, ReentrancyGuard, IShiftCallbackReceiver {
    ...
    function _handleGlvDepositError(bytes32 key, uint256 startingGas, bytes memory reasonBytes) internal {
        ...
        GlvDepositUtils.cancelGlvDeposit(
            dataStore, eventEmitter, glvVault, key, msg.sender, startingGas, reason, reasonBytes
        );
    }
    ...
    function cancelGlvDeposit(bytes32 key) external globalNonReentrant onlyController {
        ...
        GlvDepositUtils.cancelGlvDeposit(
            _dataStore, eventEmitter, glvVault, key, msg.sender, startingGas, Keys.USER_INITIATED_CANCEL, ""
        );
    }
    ...
    function shift(address account, address glv, uint256 marketTokenAmount, ShiftUtils.CreateShiftParams memory params)
        external
        globalNonReentrant
        onlyOrderKeeper
    {
