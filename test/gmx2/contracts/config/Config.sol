...
contract Config is ReentrancyGuard, RoleModule, BasicMulticall {
    ...
    function setFundingRate(address market, uint256 maxFundingFactorPerSecond) external onlyKeeper nonReentrant {
        ...
        eventEmitter.emitEventLog1("ConfigSetFundingRate", Cast.toBytes32(market), eventData);
    }
    ...
    function setDataStream(address token, bytes32 feedId, uint256 dataStreamMultiplier)
        external
        onlyTimelockAdmin
        nonReentrant
    {
    ...
    function setClaimableCollateralFactorForTime(address market, address token, uint256 timeKey, uint256 factor)
        external
        onlyConfigKeeper
        nonReentrant
    {
        ...
        eventEmitter.emitEventLog2(
            "SetClaimableCollateralFactorForTime", Cast.toBytes32(market), Cast.toBytes32(token), eventData
        );
    }
    ...
    function setClaimableCollateralFactorForAccount(
        ...
        eventEmitter.emitEventLog2(
            "SetClaimableCollateralFactorForAccount", Cast.toBytes32(market), Cast.toBytes32(token), eventData
        );
    }
    ...
    function _validateRange(bytes32 baseKey, uint256 value) internal pure {
        ...
        if (baseKey == Keys.MAX_FUNDING_FACTOR_PER_SECOND || baseKey == Keys.MAX_FUNDING_FACTOR_PER_SECOND_LIMIT) {
        ...
        if (
            baseKey == Keys.BORROWING_FACTOR || baseKey == Keys.BASE_BORROWING_FACTOR
        ...
        if (
            baseKey == Keys.FUNDING_FACTOR || baseKey == Keys.BORROWING_FACTOR
                || baseKey == Keys.FUNDING_INCREASE_FACTOR_PER_SECOND || baseKey == Keys.FUNDING_DECREASE_FACTOR_PER_SECOND
        ...
        if (
            baseKey == Keys.SWAP_FEE_FACTOR || baseKey == Keys.POSITION_FEE_FACTOR || baseKey == Keys.MAX_UI_FEE_FACTOR
        ...
        if (
            baseKey == Keys.POSITION_FEE_RECEIVER_FACTOR || baseKey == Keys.SWAP_FEE_RECEIVER_FACTOR
                || baseKey == Keys.BORROWING_FEE_RECEIVER_FACTOR || baseKey == Keys.MAX_PNL_FACTOR
                || baseKey == Keys.MIN_PNL_FACTOR_AFTER_ADL || baseKey == Keys.OPTIMAL_USAGE_FACTOR
        ) {
            // revert if value > 100%
