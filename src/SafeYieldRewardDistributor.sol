    ...
    Ownable2Step
{
    ...
    mapping(address contract_ => uint256 outstanding)
        public outStandingContractRewards;
    ...
    event SafeStakingUpdated(
        address indexed previousStaking,
        address indexed newStaking
    );
    event TeamOperationsUpdated(
        address indexed previousTeamOperations,
        address indexed newTeamOperations
    );
    event UsdcBuybackUpdated(
        address indexed previousUsdcBuyback,
        address indexed newUsdcBuyback
    );

    event RewardDistributed(
        address indexed contract_,
        uint256 indexed usdcDistributed
    );
    /*//////////////////////////////////////////////////////////////
                                 ERRORS
    //////////////////////////////////////////////////////////////*/
    ...
    */
    ...
    constructor(
        ...
        if (
            ...
            _teamOperations == address(0) ||
            _usdcBuyback == address(0) ||
            _safeStaking == address(0) ||
    ...
    function addContract(
        ...
        if (
            index != 0 ||
            newContractInfo.contract_ == approvedContracts[0].contract_
        ) {
        ...
        for (; i < updateLength; ++i) {
            ...
            if (
                approvedContracts[i].contract_ !=
                updatedAllocations[i].contract_
            ) {
            ...
            if (approvedContracts[i].share != updatedAllocations[i].share) {
                ...
                approvedContracts[i].shareDebt = SafeCast.toInt256(
                    updatedAllocations[i].share * accumulatedUsdcPerContract
                );
            }
        ...
        newContractInfo.shareDebt = SafeCast.toInt256(
            newContractInfo.share * accumulatedUsdcPerContract
        );
    ...
    function removeContract(
        ...
        ContractShare memory lastContract = approvedContracts[
            lastContractIndex
        ];
    ...
    function distributeToContract(
        address contract_
    ) public returns (uint256 usdcDistributed) {
        if (msg.sender != owner() && msg.sender != contract_)
            revert SYRD__NOT_ADMIN_OR_VALID_CONTRACT();

        updateAllocations();

        ContractShare memory contractDetails = approvedContracts[
            contractIndex[contract_]
        ];
        if (contractDetails.contract_ != contract_)
            revert SYRD__INVALID_CONTRACT();

        int256 accumulatedContractUsdc = SafeCast.toInt256(
        ...
        if (usdcDistributed != 0) {
            approvedContracts[contractIndex[contract_]]
                .shareDebt = accumulatedContractUsdc;
     ...
     */
    function switchSharesPerPhase() public {
    ...
    function removeExcessUsdc(uint256 amount) public onlyOwner {
        if (!usdcToken.transfer(owner(), amount))
            revert SYRD__TRANSFER_FAILED();
    }

    /// @dev Returns the pending rewards for a contract.
    function pendingRewards(address contract_) public view returns (uint256) {
        if (
            contract_ != approvedContracts[contractIndex[contract_]].contract_
        ) {
            return 0;
        }
        ...
        int256 accumulatedContractUsdc = SafeCast.toInt256(
            contractDetails.share * accUsdc
        );
        uint256 pendingContractUsdc = SafeCast.toUint256(
            accumulatedContractUsdc - contractDetails.shareDebt
        );
    ...
    function _distributeToAllContracts() internal {
        ...
        for (; i < allContracts; ++i) {
            ...
            int256 accumulatedContractUsdc = SafeCast.toInt256(
                contractDetails.share * uint256(accumulatedUsdcPerContract)
            );
            uint256 usdcDistributed = uint256(
                accumulatedContractUsdc - contractDetails.shareDebt
            );

            if (usdcDistributed != 0) {
                approvedContracts[i].shareDebt = accumulatedContractUsdc;
                outStandingContractRewards[
